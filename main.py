...
def checkSimilarity(window, path1, path2):
    ...
    if(result <= THRESHOLD):
        ...
        window.destroy()
        root = tk.Tk()
        root.title("signature lock")
        root.geometry("500x700")
        img2_message = tk.Label(root, text="you signed in", font=10)
...
img2_message = tk.Label(root, text="Signature", font=10)
img2_message.place(x=10, y=250)
...
compare_button = tk.Button(
    root, text="Compare", font=10, command=lambda: checkSimilarity(window=root,
                                                                   path1='Signature-Matching-main/22.png',
